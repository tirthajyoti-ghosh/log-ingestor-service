...
const enqueueBatch = async (batch: Log[]) => {
    ...
    const queueUrl = process.env.DEV ? 'http://localhost:4747' : 'https://zeplo.to';
    await axios.post(`${queueUrl}/${process.env.DB_LAYER_URL}/insert?_token=${process.env.ZEPLO_TOKEN}&_retry=3&_delay=1`, batch, {
        headers: {
